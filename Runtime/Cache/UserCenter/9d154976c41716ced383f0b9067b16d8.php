...
<div class="row">
    ...
    <div class="row">
        <div class="col-xs-12">
            <p class="text-muted">共有 <span class="text-success"><?php echo ($totalCount); ?></span> 个收藏帖子</p>
            <hr/>
        </div>
    </div>

    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$document): $mod = ($i % 2 );++$i;?><div class="row">
            <div class="col-xs-12">
                <p class="pull-right">
                    <a class="link-danger" href="<?php echo U('Forum/Index/doBookmark?add=0',array('post_id'=>$document['id']));?>">取消收藏</a>
                </p>
                <p>
                    <a class="link-black" href="<?php echo U('Forum/Index/detail',array('id'=>$document['id']));?>"><?php echo (htmlspecialchars($document["title"])); ?></a>
                </p>
                <p class="pull-right text-muted">阅读（<?php echo ($document["view_count"]); ?>）<span style="display: inline-block; width: 2em;">&nbsp;</span>回复（<?php echo ($document["reply_count"]); ?>）</p>
                <p class="text-muted"><?php echo (time_format($document["create_time"])); ?></p>
            </div>
        </div>
